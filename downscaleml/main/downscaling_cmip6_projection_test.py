...
if __name__ == '__main__':
    ...
    prediction = np.ones(shape=(predictors_valid.shape[2], predictors_valid.shape[1], predictors_valid.shape[0])) * np.nan
    LogConfig.init_log('Shape is in (spatial, time, variables):{}'.format(prediction.shape))
    ...
    for i in range(predictors_train.shape[0]):
        for j in range(predictors_train.shape[1]):
            ...
            LogConfig.init_log('Processing grid point: {:d}, {:d} '.format(i, j))
    ...
    predictions = xr.DataArray(data=prediction, dims=['time', 'y', 'x'],
                            coords=dict(time=pd.date_range(Cmip6_valid.time.values[0],Cmip6_valid.time.values[-1], freq='D'),
                                        lat=Cmip6_valid.y, lon=Cmip6_valid.x))
